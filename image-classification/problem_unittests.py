...
def test_one_hot_encode(one_hot_encode):
    ...
    for one_hot in new_enc_labels:
        assert (
            one_hot == 1
        ).sum() == 1, f'Each one-hot-encoded value should include the number 1 exactly once.\nFound {one_hot}\n'
        assert (one_hot == 0).sum() == len(
            one_hot
        ) - 1, f'Each one-hot-encoded value should include zeros in all but one position.\nFound {one_hot}\n'
